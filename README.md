...
一个仅使用 Python 标准库的 Pixiv CDN 探测工具。它从相邻作品推断投稿时间，枚举可能的原图 URL，并自动下载仍由 CDN 公开提供的多页图片。
...
该方法只对 CDN 尚未清理、且文件名不含不可枚举哈希的近期作品有效。脚本不会绕过登录、付费墙或其他访问控制，只请求 Pixiv CDN 仍公开返回的 URL。

请仅恢复你有权保存的内容，尊重作者删除作品的决定，并遵守所在地法律及 Pixiv 服务条款。不要将恢复内容提交到本仓库。
